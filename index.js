...
//nestedFunction() can use closure to reach out and access variables in myFunction()
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(zooArr){
    const lowerArr = zooArr.map(element => element.animal_name.toLowerCase());
    return lowerArr;
  }
  ...
  */
  ...
  function lowPopulationAnimals(zooArr){
    const lowPOP = zooArr.filter(element => element.population < 5);
    return lowPOP;
  }
  ...
  */
  ...
  function USApop(zooArr){
    const totalPop = zooArr.reduce((total, element) => total + element.population ,0)
    return totalPop;
  }
